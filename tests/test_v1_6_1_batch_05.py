...
ENTITY_IDS = {"ckb:weapon:firearm:ppsh_41", "ckb:ammunition:cartridge:7_62x25_tokarev"}
RELATIONSHIP_ID = "rel:v1_6_1_batch_05:ppsh_41:uses_ammunition:7_62x25_tokarev"

class SmallArmsBatch05Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_batch_registers_two_entities_and_relation(self):
        batch = next(row for row in self.batches if row.get("batch_id") == "v1.6.1-small-arms-batch-05-ppsh-41")
    ...
    def test_relation_is_independent_and_source_checked(self):
        ...
        self.assertEqual(relationship.source_id, "ckb:weapon:firearm:ppsh_41")
        self.assertEqual(relationship.target_id, "ckb:ammunition:cartridge:7_62x25_tokarev")
    ...
    def test_ammunition_profile_excludes_loading_recipe_fields(self):
        ...
        fields = {claim.get("field") for claim in self.entity_map["ckb:ammunition:cartridge:7_62x25_tokarev"].technical["claims"]}
