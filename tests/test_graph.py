...
class KnowledgeGraphTests(unittest.TestCase):
    def test_neighbors_and_shortest_path(self):
        ...
        self.assertEqual(graph.neighbors(a.id, direction="out"), [ab])
        self.assertEqual(graph.shortest_path(a.id, c.id), [ab, bc])

    def test_missing_target_and_invalid_confidence_are_rejected(self):
        a = entity("ckb:test:a")
        ...
        self.assertTrue(any("missing target entity" in error for error in errors))
        self.assertTrue(any("confidence outside" in error for error in errors))

    def test_invalid_neighbor_direction_is_rejected(self):
        a = entity("ckb:test:a")
        graph = KnowledgeGraph([a], [])
