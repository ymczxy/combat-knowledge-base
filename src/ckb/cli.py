...
def main() -> None:
    ...
    if args.cmd == "stats":
        ...
        for key, value in sorted(Counter(e.classification.get("domain", "Unknown") for e in entities).items()): print(f"  {key}: {value}")
        stats = catalog_stats(load_catalog(ROOT / "data" / "catalog"))
        print("catalog_candidates:", stats["total"])
        for key, value in stats["priorities"].items(): print(f"  {key}: {value}")
        return
    ...
    if args.cmd == "source-audit":
        ...
        raise SystemExit(_print_errors(errors))
    errors = validate_all(entities)
    if errors: raise SystemExit(_print_errors(errors))
    args.output.mkdir(parents=True, exist_ok=True)
    build_sqlite(entities, args.output / "ckb.sqlite")
    build_markdown(entities, args.output / "markdown")
    (args.output / "ckb.json").write_text(json.dumps([e.raw for e in entities], ensure_ascii=False, indent=2), encoding="utf-8")
    if args.profile:
        profile = load_profile(ROOT / "data" / "curated" / args.profile / "build_profile.json")
